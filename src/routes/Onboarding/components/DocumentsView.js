          ...
          {documentName ?
            <div className="ui labeled button">
              <Button type="button" onClick={openConfirmDeleteModal}>
                {isDeleting ? <div className="ui active mini inline loader"/> : <i className="trash icon"></i>}
              </Button>
              <a className="ui basic label" href={documentUrl} target="_blank">{documentName}</a>
            </div>
          : null}
          {!documentName ? 
            <FileButton type="button" accept="image/*, application/pdf" basic={true} onChange={onFileChange}>
